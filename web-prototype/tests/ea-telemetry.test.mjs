...
assert.match(ea, /#property version\s+"4\.00"/);
assert.match(ea, /TradingNote_MT4_Journal_v4\.tsv/);
assert.match(ea, /TradingNote_MT4_OpenState_v4\.tsv/);
assert.match(ea, /TradingNote_MT4_ExportedTickets_v4\.txt/);
assert.match(ea, /values\[0\] = "4"/);
...
assert.match(ea, /symbol == "NAS100\.R"[\s\S]*return MathAbs\(priceDistance\) \* lots;/);
assert.match(ea, /isCashIndexR \? tracker\.initialRiskPrice : tracker\.initialRiskPrice \/ PointSize/);
