...
assert.match(html, /id="mcRisk"[^>]*min="0\.01"[^>]*step="0\.01"[^>]*value="1"/);
assert.match(html, /id="toleranceInput"[^>]*min="0\.05"[^>]*max="2"[^>]*step="0\.05"/);
assert.match(app, /tolerance:'tradingnote\.analysisTolerance'/);
assert.match(app, /localStorage\.setItem\(KEYS\.tolerance,String\(tolerance\)\)/);
assert.match(app, /function consistencyScore\(/);
assert.match(app, /三層一致性評分/);
