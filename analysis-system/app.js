...
function renderMonteCarloComparison(results){
  const ready=Object.values(results).every(result=>result?.worstRun);if(!ready){$('#mcComparison').innerHTML='<div class="empty-inline">三層各自至少需要 5 筆非 0R，才能產生完整比較。</div>';return}const rows=[['P50 期末 R','p50FinalR',value=>signed(value)],['P10 期末 R','p10FinalR',value=>signed(value)],['99% R 回撤','p99RDrawdown',value=>`-${value.toFixed(2)}R`],['99% 連敗壓力','p99LossStreak',value=>`${Math.ceil(value)} 連敗`],['虧損機率','lossRate',value=>`${value.toFixed(1)}%`]];$('#mcComparison').innerHTML=`<div class="mc-compare-head"><span>指標</span><b>回測</b><b>復盤</b><b>實盤</b></div>${rows.map(([label,key,format])=>`<div><span>${label}</span><b>${format(results.Backtest[key])}</b><b>${format(results.Review[key])}</b><b>${format(results.Live[key])}</b></div>`).join('')}`;
}
